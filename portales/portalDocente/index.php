...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "SELECT EXPEDIENTE_DOCENTE, NIP_DOCENTE FROM DOCENTES WHERE EXPEDIENTE_DOCENTE = ?";
    ...
    if ($expediente_db && password_verify($_POST['nip'], $nip_hash_db)) {
        ...
        $_SESSION['login'] = true;
        header('Location: propiedadesDocente/principalDocente.php?expediente='.$expediente);
        exit();
    } else {
        // Usuario no encontrado o contraseña incorrecta, mostrar mensaje de error
        $errores[] = "El usuario no existe o las credenciales son incorrectas";
    }
}
?>
