...
$query = "SELECT EXPEDIENTE_DOCENTE, NIP_DOCENTE FROM DOCENTES";
$resultado = mysqli_query($db, $query);

// Hashear cada contraseña y actualizar la base de datos
while ($usuario = mysqli_fetch_assoc($resultado)) {
    $expediente = $usuario['EXPEDIENTE_DOCENTE'];
    $nip = $usuario['NIP_DOCENTE'];
    
    // Hashear la contraseña
    $nip_hash = password_hash($nip, PASSWORD_DEFAULT);
    
    // Actualizar la base de datos con la contraseña hasheada
    $update_query = "UPDATE DOCENTES SET NIP_DOCENTE = '$nip_hash' WHERE EXPEDIENTE_DOCENTE = '$expediente'";
